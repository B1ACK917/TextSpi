    ...
    def get_precontent_with_proxy(self, url, index):
        ...
        while True:
            try:
                ...
                self.content.append((pre_content[0].get_text()[:-98].replace('    ', '\n\n    '), index))
                break
    ...
    def run(self):
        ...
        if not os.path.exists('output'):
            ...
            if not os.path.exists(os.path.join('output', 'epub')):
                os.mkdir(os.path.join('output', 'epub'))
        pb = ProgressBar()
        with open(os.path.join('output', 'epub', self.output), 'w', encoding='utf-8') as file:
            for i in pb(range(self.catalog_nums)):
                file.write(self.catalog[i])
                file.write(self.content[i][0])
                file.write('\n\n')
